...
public class DatabaseController {
    ...
    public ResponseEntity<Map<String, Object>> migrateTransactions() {
        ...
        try { jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_txn_buyer ON transactions(buyer_id)"); } catch (Exception e) {}
        try { jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_txn_seller ON transactions(seller_id)"); } catch (Exception e) {}
        return ResponseEntity.ok(Map.of("success", true, "results", results));
    }
    ...
    public ResponseEntity<Map<String, Object>> createAdmin() {
        ...
        try {
            ...
            else { String encodedPassword = passwordEncoder.encode("Admin@123"); jdbcTemplate.update("INSERT INTO users (email, phone, password, first_name, last_name, user_type, status, account_type, commission_rate, wallet_balance, total_earned, referral_code, created_at, updated_at) VALUES ('admin@cambiz.cm', '670000000', ?, 'Admin', 'User', 'ADMIN', 'ACTIVE', 'REGULAR', 0, 0, 0, 'ADMIN1', NOW(), NOW())", encodedPassword); results.add("Admin created: admin@cambiz.cm / Admin@123"); }
    ...
    public ResponseEntity<Map<String, Object>> addApprovalColumns() {
        ...
        try { jdbcTemplate.execute("UPDATE products SET status = 'APPROVED' WHERE is_approved = true AND status = 'PENDING_APPROVAL'"); results.add("Synced existing products"); } catch (Exception e) {}
    ...
    public ResponseEntity<Map<String, Object>> addCategoryActive() {
        List<String> results = new ArrayList<>();
        try { jdbcTemplate.execute("ALTER TABLE categories ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE"); results.add("is_active added to categories"); } catch (Exception e) { results.add("is_active: " + e.getMessage()); }
        return ResponseEntity.ok(Map.of("success", true, "results", results));
    }
    ...
    public ResponseEntity<Map<String, Object>> createSettingsTable() {
        List<String> results = new ArrayList<>();
        try {
            jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS platform_settings (id BIGSERIAL PRIMARY KEY, setting_key VARCHAR(100) UNIQUE NOT NULL, setting_value TEXT, description VARCHAR(255), created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW())");
            results.add("platform_settings table created");
